...
export const generatePushMessage = ({ templateData, channel, payload, priority }: TemplateInput): PushMessage => {
  ...
  let body = `Entrega #${templateData.deliveryNumber}: ${statusParts.line1.substring(0, 80)}${statusParts.line1.length > 80 ? '...' : ''}`;
  if (statusParts.line2 && (body.length + statusParts.line2.length < 120)) {
    body += ` ${statusParts.line2.split('.')[0]}.`;
  }
