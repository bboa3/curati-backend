...
export const generateEmailMessage = ({ templateData, channel, payload }: TemplateInput): EmailMessage => {
  ...
  const preheaderText = `Fatura #${templateData.invoiceNumber} de ${formatToMZN(templateData.invoiceTotalAmount)} ${templateData.invoiceStatus === InvoiceStatus.PENDING_PAYMENT ? 'pronta para pagamento.' : 'disponível.'}`;

  let html = generateEmailHeader({ brandConfig, preheaderText });

  html += `
    <h2 style="margin:0 0 16px;">${textParts.emailTitle}</h2>
    <p style="font-size:14px;color:${brandConfig.colors.BLACK2};margin:0 0 20px;">Emissão: ${formattedInvoiceDate} | Vencimento: ${formattedDueDate}</p>
    <p>${textParts.greeting}</p>
    <p>${textParts.line1}</p>
    ${textParts.invoiceSourceDescription ? `<p style="font-size:14px;color:${brandConfig.colors.BLACK2};margin:0 0 20px;">${textParts.invoiceSourceDescription}</p>` : ''}

    <table width="100%" border="0" cellpadding="5" cellspacing="0" role="presentation" style="margin:20px 0;border-collapse:collapse;">
      <tr>
      ...
      <tr>
        <td colspan="2"><hr style="border:none;border-top:1px solid ${brandConfig.colors.BLACK3};margin:5px 0;"/></td>
      </tr>
      <tr>
        <td style="font-weight:bold;color:${brandConfig.colors.BLACK};">Total a Pagar:</td>
        <td style="text-align:right;font-weight:bold;color:${brandConfig.colors.PRIMARY};">${formatToMZN(templateData.invoiceTotalAmount)}</td>
      </tr>
    </table>

    <p style="margin:10px 0;font-weight:bold;">Estado Atual: <span style="color:${templateData.invoiceStatus === InvoiceStatus.PAID ? brandConfig.colors.GREEN : templateData.invoiceStatus === InvoiceStatus.PENDING_PAYMENT ? brandConfig.colors.ORANGE : brandConfig.colors.BLACK2};">${convertInvoiceStatus(templateData.invoiceStatus)}</span></p>
    ${templateData.paymentTerms ? `<p style="font-size:13px;color:${brandConfig.colors.BLACK2};margin:0 0 20px;">Termos: ${convertPaymentTermsType(templateData.paymentTerms)}</p>` : ''}
    ...
    <p style="font-size:13px;">Se tiver alguma dúvida, <a href="mailto:${brandConfig.supportEmail}" style="color:${brandConfig.colors.PRIMARY};text-decoration:none;">contacte o nosso suporte</a>.</p>
  ...
  if (templateData.totalDeliveryFee && templateData.invoiceSourceType === InvoiceSourceType.MEDICINE_ORDER) textBody += `\n- Taxa de Entrega: ${formatToMZN(templateData.totalDeliveryFee)}`;
  ...
  if (templateData.invoiceDocumentUrl && textParts.buttonTextDownloadPdf) textBody += `\n${textParts.buttonTextDownloadPdf}: ${templateData.invoiceDocumentUrl}`;
