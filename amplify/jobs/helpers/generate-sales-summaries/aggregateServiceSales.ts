...
export const aggregateServiceSales = async ({ businessId, periodStart, periodEnd, dbClient }: AggregatorInput) => {
  ...
  if (consultationsErrors) throw new Error(`Failed to fetch consultations: ${JSON.stringify(consultationsErrors)}`);
  ...
  if (servicesErrors) throw new Error(`Failed to fetch services: ${JSON.stringify(servicesErrors)}`);
  ...
  if (pricingErrors) throw new Error(`Failed to fetch pricing: ${JSON.stringify(pricingErrors)}`);
